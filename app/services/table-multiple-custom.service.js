(function () {
    ...
    angular
        .module('altairApp')
        .factory('TableMultipleCustom', TableMultipleCustom);

    TableMultipleCustom.$inject = ['$translate'];

    function TableMultipleCustom($translate, $scope) {
        var service = {
        ...
        function main() {
            $scope.setPageDisplay = function (table_id) {
                var left = ($scope.TABLES[table_id].param_current_page-1) * $scope.TABLES[table_id].param_page_size + 1;
                var right;
                if ($scope.TABLES[table_id].param_current_page == $scope.TABLES[table_id].param_page_total) {
                    right = $scope.TABLES[table_id].param_total_result;
                } else {
                    right = ($scope.TABLES[table_id].param_current_page) * $scope.TABLES[table_id].param_page_size;
                }
                ...
                if (!allowReloadDropSelectPage) {
                    $scope.TABLES[table_id].selectize_page = $scope.TABLES[table_id].param_current_page;
                }
                ...
                if($scope.TABLES[table_id].param_page_total == 1) {
                    ...
                    if($scope.TABLES[table_id].param_current_page == 1) {
                        $("#" + $scope.TABLES[table_id].pager_id).find('.firstPage').addClass('disableMouse');
                        $("#" + $scope.TABLES[table_id].pager_id).find('.prevPage').addClass('disableMouse');
                    }
            ...
            $scope.resetPageDisplay = function (table_id) {
                if($scope.TABLES[table_id].param_page_size>0){
                    ...
                    $scope.TABLES[table_id].param_current_page = 1;
            ...
            $scope.handleLastPage = function (table_id) {
                ...
                $scope.TABLES[table_id].param_current_page = $scope.TABLES[table_id].param_page_total;
                allowReloadDropSelectPage = false;
                $scope.reloadPage(table_id);
            }
            ...
            $scope.selectPageHandle = function (table_id) {
                if (allowReloadDropSelectPage) {
                    $scope.TABLES[table_id].param_current_page = $("#"+$scope.TABLES[table_id].selectize_page_id).val();
                    $scope.reloadPage(table_id);
                }
            }
